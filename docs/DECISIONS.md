...
## 2026-07-21 — Zombies stand on the ground band: getItemTopPosition narrowed from full-canvas to a feet-in-band range (Cowork session, Opus)

**Decision:** Closed the V1 follow-up split off during the parallax session. `Movement.getItemTopPosition` no longer randomizes a top-level item's vertical position across the full canvas (`randomFn() * (canvasHeight - itemHeight)`). It now places the item's FEET — its sprite box bottom, the same alignment line sub-tasks already bottom-align to — at `feetTop + randomFn() * (feetBottom - feetTop)`, where `feetTop = CONFIG.GROUND_BAND_FEET_TOP_FRAC * canvasHeight` (0.85) and `feetBottom = CONFIG.GROUND_BAND_FEET_BOTTOM_FRAC * canvasHeight` (0.99); `top = feet - itemHeight`, clamped to `[0, canvasHeight - itemHeight]`. Two new tunable layout constants in `js/config.js`.

**Why these numbers:** the `.scene-ground` band is the bottom 24% of the canvas (frac 0.76–1.0); the fence silhouette sits near its top (≈0.82–0.86 of canvas height, measured live). Feet in [0.85, 0.99] puts zombies from right at the fence line down to just above the canvas floor — standing in front of the fence, not floating above it. 0.85 rather than 0.76 (band top) so a zombie's feet never land above the fence (which would read as standing behind it).

**Overlap — the actual judgment call the split was for:** narrowing the range necessarily clusters items vertically, so the risk was same-timeline enemies (identical due time → identical x) stacking exactly on top of each other. The ~14% jitter (≈56px on a 401px canvas) resolves it: same-x items land at different feet heights and stagger for depth instead of overlapping fully. Verified live in Chrome — two tasks both due 11:59 PM spawned at x=655 with feet at frac 0.893 and 0.961 (27px apart, both in-band); a 2,000-sample sweep of the real function landed every foot inside [0.85, 0.99] with 56px observed spread. Judged acceptable: a real crowd on a ground line overlaps somewhat anyway, and horizontal x is due-time-driven so exact-same-time collisions are the uncommon worst case, not the norm.

**Alternatives considered:** (a) a fixed feet line with no jitter — rejected, same-time items would perfectly overlap and read as one zombie; (b) coupling the band to the actual `.scene-ground` DOM rect instead of canvas-height fractions — rejected as unnecessary CSS/JS coupling (the function only receives `canvasHeight`; matching fractions to the CSS 24% band by hand is simpler and the numbers are tunable if the band height ever changes); (c) adding vertical sprite margins to CONFIG so "feet" accounts for transparent PNG bottom padding — unneeded, live check confirmed the boot graphic sits ~at the box bottom (consistent with how sub-task bottom-alignment already treats box-bottom as the feet line).

**Verification:** 60 suites, 1271/1271 (+1; `test/movement.test.js` updated — the two full-canvas-range assertions rewritten to the band, plus a new full-jitter-range bounds test). Jest ran clean this session from `$HOME` (the prior session's sandbox jest wall did NOT recur). `node --check` implicit via successful require in tests. Live-verified in Chrome against Jeremy's real `node server.js` per above; zero console errors. Test task created for the two-zombie check was removed cleanly afterward via a localStorage edit + a `localStorage.setItem` write-guard through the reload (needed because Persistence/saveGame/State are all closure-scoped — not on `window` — so the usual stub-the-flush trick couldn't reach them; blocking the save-key write at the `setItem` level did, and left Jeremy's real save — 1 task, XP/Points 10, Health 100 — untouched).

---

## 2026-07-21 — V1 live-tuning pass: ground now moves (fastest of all layers, supersedes "ground fixed" below); clouds brightened, sped up, and rebuilt as a seamless wrap-around loop (Cowork session, Sonnet, several rounds of Jeremy's live feedback in Chrome)

**Decision: "ground fixed" (the entry directly below) is SUPERSEDED.** Jeremy's observation, watching the scene live: zombies stand on the ground band and visibly move as the slider scrubs, so a perfectly static ground under them read as sliding rather than walking. `.scene-ground` now gets a `translateX(calc((var(--scene-progress, 0.5) - 0.5) * -22%))` — the FASTEST rate of any layer (clouds `-18%`, silhouettes `-7%`), widened to `130%/-15%` (same margin trick as clouds) so the swing never reveals a canvas-edge gap. This also happens to match real-world parallax physics (the closest layer to the viewer moves the most) rather than the original stylized inversion — the original call wasn't wrong on its own terms, it was just optimizing for a different feel than what reads correctly with zombies actually walking on top of it.

**Cloud tuning, four rounds in one session (all `css/gameCanvas.css`, one small `js/ui/timeSliderView.js` addition):**
1. Brightened + parallax widened (clouds `-9%→-18%`, silhouettes `-3%→-7%`) so the slider-driven motion reads clearly, not just the ground/midnight-line.
2. Idle "wind drift" added: brightened again, sped up `70s→12s`.
3. Rebuilt from a bounded `transform: translateX` wiggle (visibly snapped back at its endpoints) into a seamless `background-position` loop on a `repeat-x` tile — once position has moved exactly one tile width the pattern is pixel-identical to its start, so clouds now sail off the right edge and reappear on the left with no pop, and no oversized-box hack was needed anymore.
4. Tile widened `500px→1100px` with shape count `4→2` (Jeremy: too many clouds at once on a wide canvas), duration scaled `7s→15s` in the same step specifically to hold px/sec speed constant — this round was about density, not pace, so pace was deliberately not re-litigated.

**Scrub-lock, added alongside the loop rebuild:** `js/ui/timeSliderView.js`'s `handleScrubInput`/`handleRelease` toggle a new `.parallax-scrubbing` class on `#graveyardScene`; `css/gameCanvas.css` pauses the idle-drift animation (`animation-play-state: paused`) while that class is present. Without this, the idle wind drift and the slider-driven parallax transform would both be moving the clouds at once during a scrub, muddying the "world responds to scrubbing" signal the parallax exists to deliver — Jeremy's framing was "positions should lock for the parallax effect when the slider is used," i.e. only ONE motion source should be live at a time.

**Verification:** live in Chrome only, same session as the V1 entry below (no new jest run — that sandbox limitation was already logged and didn't change mid-session). Confirmed via direct slider-value sweeps + `getComputedStyle` reads: ground's `translateX` swings correctly and stays gapless at both extremes; the cloud loop's `animationPlayState` flips to `paused` exactly when `.parallax-scrubbing` is present and back to `running` on release; screenshots a few seconds apart confirm continuous rightward cloud motion at the tuned speed with visibly fewer clouds after the density pass.

**Not yet reconciled:** `docs/UI_UX.md`'s V1 section still describes the ORIGINAL numbers/"ground fixed" framing from before this tuning pass — flagged in `sessions/HANDOFF.md` rather than rewritten here line-by-line (would need re-deriving exact current values from CSS); a future doc pass should fold this entry's numbers into that section properly.

---

## 2026-07-21 — V1 shipped: graveyard atmosphere + slider-coupled parallax; ground-band zombie alignment deliberately split off (Cowork session, Sonnet)

**Decision:** Executed VISUAL_DESIGN_PLAN.md's V1 item (fully spec'd by the 2026-07-21 art-direction Fable session — see that entry below). New `.graveyard-scene` layer stack in `#gameCanvas` (`css/gameCanvas.css`, `index.html`): night sky + static moon, faint clouds, a far hill + 3 bare-tree silhouettes and a fence + 3 tombstones (both inline SVG, no new binary asset), midnight line restyled from a red alarm hairline to a dark iron gate-post with a lantern glow. All negative z-index so nothing else's z-index needed to change.

**Stacking-context bug found + fixed live in Chrome:** the scene rendered nothing on first load despite every layer having correct computed styles/rects (verified via `getBoundingClientRect`, fill colors, etc.) — the negative-z-index children of `.graveyard-scene` require their positioned ancestor to establish its own stacking context to stay scoped "behind `#gameCanvas`'s own children" as intended; `#gameCanvas` was `position: relative` with no `z-index`, so it never became a stacking-context root, and the negative-z layers escaped upward past it entirely instead. Fixed with one line, `#gameCanvas { z-index: 0; }`. Logged in `docs/UI_UX.md` as a general pattern for any future negative-z decorative layer, not just this one.

**Parallax mechanism:** `js/ui/timeSliderView.js` sets `--scene-progress` (0–1) on `#graveyardScene` from both the live tick (`syncHandle`) and an active scrub (`handleScrubInput`), so the scene always reflects whatever time is currently on screen rather than only reacting to a manual scrub. Deliberately NOT threaded through the module's deps contract — it's a purely decorative DOM query, and every existing test's `makeDeps()` would otherwise need a fake collaborator for a feature with no game-state coupling. `.scene-clouds`/`.scene-silhouettes` read the var via `translateX(calc((var(--scene-progress, 0.5) - 0.5) * rate))`; `.scene-ground` is intentionally NOT transformed at all (fixed — the stillness against the moving layers above it is what reads as depth, per the 2026-07-21 art-direction decision). Idle cloud drift + ground fog are separate keyframe animations on their own elements so they never fight the parallax `transform` on the same property/element; both gated behind `fx-off`/`fx-reduced` + `prefers-reduced-motion`.

**Ground-band zombie alignment — deliberately NOT done this session:** the V1 task itself says "verify `getItemTopPosition`'s lane offsets still separate clusters; if the lane math itself must change, STOP and split a follow-up." Checked before writing any CSS: `Movement.getItemTopPosition` randomizes a top-level item's vertical position across the FULL canvas height (`randomFn() * (canvasHeight - itemHeight)`), not a bottom band — there is no narrow range to just visually align to. Narrowing that range to sit on the new ground band is a real lane-math change (increases overlap risk with several simultaneous active items, which is exactly the kind of judgment call the stop condition exists for), so it's logged as its own ROADMAP follow-up rather than decided unilaterally in a session that was scoped as CSS/JS-decoration execution. The visual ground band is real and ready for it.

**Alternatives considered:** giving each element that currently has no explicit z-index (`#base`, `#heroBaseZone`, `.enemy`) a small positive z-index instead of fixing `#gameCanvas` — rejected as needlessly wider blast radius; a single `z-index:0` on the shared positioned ancestor achieves the same stacking order with a one-line change and zero risk to those other elements' existing behavior. Building the props as a separate cached `Assets/scene/*.svg` file instead of inline SVG in `index.html` — rejected for this pass since it would need `sw.js`'s `APP_SHELL` array updated for a genuinely new cacheable asset; inline SVG needed only the existing `CACHE_NAME` version bump (index.html itself was already going to change).

**Verification:** `node --check` clean on `js/ui/timeSliderView.js`; module load-tested via plain `require()` (confirms no syntax/runtime error). **`npx jest` could not be run this session** — a NEW sandbox-environment finding, not related to this session's code: even a maximally minimal fresh `jest.config.js` + trivial test in a brand-new sandbox directory fails jest's own config-validation resolver (`Module .../test/setup.js ... was not found`) for a file that `fs.existsSync`/`require()`/`graceful-fs` all confirm exists and is readable — reproduced from scratch outside this repo entirely, so it's environment infrastructure, not a code regression. Flagged for a future session in case it recurs; today's verification leaned entirely on live Chrome + `node --check` instead. Live-verified in Chrome against Jeremy's real `node server.js`: desktop scene renders correctly (hill/trees/fence/tombstones/moon all visible); 390×844 same-origin iframe showed no page-level overflow (`.graveyard-scene`'s own `scrollWidth > clientWidth` is expected/by-design since it's `overflow:hidden` by construction, not a bug signal); `--scene-progress` and the resulting `translateX` swept correctly across the full 0–1439 minute range with `ground` staying untransformed throughout; `fx-off`/`fx-reduced` both correctly stopped the idle cloud-drift and fog animations, default left them running; hero-chip zone (base's leftmost 120px) unchanged; zero console errors beyond the standard Chrome-extension message-channel noise.

---

## 2026-07-21 — V3a shipped: agenda-row mobile overflow fixed; Reset Game gated to dev mode (Cowork session, Sonnet)

**Decision:** Executed VISUAL_DESIGN_PLAN.md's V3a item (a fully-specified functional bug + a scoped dev-chrome gate, no new design questions). Two independent fixes in one session per the batching guardrail.

**Agenda row clipping — root cause and fix:** `js/ui/agendaList.js`'s `createListItem` builds a row whose title/details lines are `display:flex` with no `flex-wrap`, and CSS flex spec makes a nowrap flex container's automatic minimum width the SUM of its children's minimum widths (not "whatever fits") — so a task with a category badge, due date, sub-task-progress label, and the "+ Sub-task" button all on one un-wrappable line overflowed the card at narrow widths, and the browser visually clipped it rather than showing a horizontal scrollbar. Two separate chains needed the fix, discovered in sequence via live `getBoundingClientRect`/`scrollWidth` measurement in Chrome (not just eyeballing a screenshot — the first fix LOOKED clean visually but still measured 49px of overflow until the second chain was found):
1. The title/details rows live inside `.item-info` but are unclassed divs (no selector to target directly) — fixed structurally via `.item-info div { flex-wrap: wrap }` in `css/responsive.css`'s existing `max-width:768px` block, plus `min-width:0`/`overflow-wrap:break-word` on `.item-name` and `min-width:0` on `.item-info` itself (`css/agendaList.css`, not breakpoint-gated — harmless at any width). `.task-controls` (edit icon + "Mark as Complete") carries an inline `flex-shrink:0` that pinned it to full unwrapped width even after wrapping onto its own line under the title — needed `flex-shrink:1 !important` (normal-weight external CSS cannot beat an inline declaration; `!important` can, regardless of origin) plus `min-width:0`.
2. Sub-tasks render as a SIBLING of `.item-info` (both children of the same `taskContentDiv`), not a descendant — so fix #1 alone left a task's rendered sub-tasks free to overflow and drag the whole card wider along with them. `.sub-task-item`/`.sub-task-info`/`.sub-task-controls` are classed (unlike the item-info-internal rows), so this one didn't need a structural selector: `flex-wrap:wrap` + `row-gap` on `.sub-task-item`, `min-width:0` on `.sub-task-info`, `flex-wrap:wrap` + `justify-content:flex-end` on `.sub-task-controls`.

**Reset Game gating:** new pure `SettingsView.isDevMode(loc)` (`js/ui/settingsView.js`, exported for tests, takes a location-like object rather than reading `window.location` directly so it's unit-testable without jsdom) — true on `localhost`/`127.0.0.1` (any port, covers `node server.js`/`npx serve .`) or when a `?dev` query param is present on ANY origin (so it's still reachable for a spot-check on the live GitHub Pages build without needing a local server). `renderSettingsWindow` now only includes the "Reset Game" section's HTML when `isDevMode()` is true. This closes a gap the 2026-07-21 "moved Reset into Settings" session (same day, earlier entry above) left open: moving it out of a floating corner button fixed the accidental-tap risk, but left it unconditionally visible to every real player on the live build, which is dev chrome that shouldn't ship.

**Alternatives considered:** gating Reset via a build-time flag/env var was rejected — this repo has no build step (plain script tags, `server.js` is a dev-only static file server), so a runtime location check is the only option that doesn't add tooling. Passing `window.location` through `deps` (matching the rest of the module's deps-injection convention) was considered over calling it directly at the render call site — rejected as unnecessary ceremony since this file already calls other browser globals directly (`Blob`, `URL.createObjectURL`, `navigator.clipboard`) without deps injection; the pure `isDevMode(loc)` core is what actually needed to be testable, and it is.

**Verification:** new `test/settings-view-devmode.test.js` (6 cases: localhost/127.0.0.1 true, `?dev` on any origin true, plain production origin false, unrelated query param false, missing/null location safely false). 60 suites, 1270/1270 (+6). `node --check` clean on `js/ui/settingsView.js`. Live-verified in Chrome against Jeremy's real `node server.js`: a same-origin iframe injected at 390×844 (the standing mobile-verification recipe) showed zero clipping on a real task with a real sub-task (category badge, due date, sub-task-progress, streak-equivalent controls all wrapping cleanly), confirmed via `scrollWidth`/`clientWidth`/`getBoundingClientRect` checks on the `li`/`.task-list-container`/`body`/`html` (zero overflow at every level — the only remaining page-level horizontal overflow was `.game-canvas`, which is the pre-existing battlefield scene, out of V3a's scope and already queued separately under V1/V3b); Settings → Reset Game correctly showed on localhost, `isDevMode` correctly returned `false` for a simulated `schooley02.github.io` hostname with no `?dev` param; Reset itself still works end-to-end (wiped the test task, board returned to onboarding empty state). Dev save reset to pristine afterward. Zero console errors throughout.
