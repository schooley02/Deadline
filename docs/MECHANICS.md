...
- Streaks: missing a habit (overdue) resets its streak to 0. **The streak is a VISUAL/legibility layer only (decided 2026-07-18 with Jeremy on Fable): agenda badge + on-fire sprite at high streak — no economy effect.** The points bonus instead comes from a rolling **success rate**: over the habit's last 14 *scheduled* occurrences (per its `schedule` — see DATA_SCHEMA.md `occurrenceHistory`), success = completed for positive habits / avoided for negative ones. Rate ≥90% → 1.5× points; ≥70% → 1.25×; else 1× (thresholds/multipliers in `js/config.js`, balance-protocol tunable; capped at 1.5× by construction). Multiplier is inactive (1×) until ≥7 scheduled occurrences are recorded, so a new habit can't instantly max out. Applies to POINTS ONLY, never XP — bonuses price into the Milestone 3 shop without accelerating levels. Uncompleting flips today's history entry and recomputes, so refunds are symmetric by construction (obsoletes the old flat-+5 refund-asymmetry bug). Replaces both the old deterministic flat +5 at streak ≥3 (live code) and this doc's former "higher chance of double points" line (never built). NOT YET IMPLEMENTED — needs the Schedule migration's occurrence data; one schemaVersion bump (2→3) covers both. See DECISIONS.md 2026-07-18.
