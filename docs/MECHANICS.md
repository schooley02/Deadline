...
- A country church overlooking an eerie graveyard. 100 HP. **Healing: GRADUAL REGEN (decided 2026-07-18 with Jeremy on Fable, resolving the 2026-07-17 doc conflict; not yet built — [P2-GAME-012]):** the base heals 1 HP per 5 min (`CONFIG.BASE_REGEN_HP` / `CONFIG.BASE_REGEN_INTERVAL_MS`, balance-protocol tunable) continuously while the run is alive, clamped at 100. Offline time heals at the same rate, applied on restore AFTER offline overdue damage is back-charged (net effect: a quiet base recovers overnight; a neglected one doesn't). Repair kits (ECONOMY.md's three tiers) are instant mid-run heals — the "I need HP now" lever on top of slow free regen. There is NO daily full reset (rejected: it zeroes the meaning of overnight damage and makes repair kits pointless), and NO "healing disabled at 0 HP until repaired" state from PROJECT_SPEC — 0 HP is and remains run over (`gameOver()`), so that clause is moot. See DECISIONS.md 2026-07-18.
