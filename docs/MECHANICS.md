...
- **Resolved 2026-07-19 (session 46, [P1-DATA-004] sequencing): BOTH effects happen together.** The original spec's growing/shrinking parent (PROJECT_SPEC.md) and the separate clustered sub-task sprites actually built (confirmed 2026-07-17) are not in tension — the parent scales up per open sub-task and shrinks back as they complete, AND the clustered sub-sprites stay. Visual sizing lands in SUBTASKS_PLAN.md sub-session 4; not yet built as of this entry.
...
- **Sub-task due dates are dependent on the parent's** (not yet built as of this entry — SUBTASKS_PLAN.md sub-session 2): default and maximum = the parent's deadline, earlier is allowed. Pulling the parent's deadline earlier re-clamps any now-later children down to it; pushing it later leaves children alone.
- **Sub-tasks currently pay the same reward as a standalone task** (10 XP/10 pts) — not yet rebalanced. SUBTASKS_PLAN.md sub-session 3 will drop this to half value (5/5) via the balance-tuning protocol.
