...
## 2026-07-21 — Zombies stand on the V1 ground band (movement.js follow-up, Cowork session, Opus)

**Did:** Closed the V1 follow-up ("put zombies on the ground band") split off during the parallax session. `Movement.getItemTopPosition` no longer randomizes a top-level item across the full canvas height — it now lands the item's FEET (sprite box bottom, same line sub-tasks bottom-align to) at a random fraction of canvas height between `CONFIG.GROUND_BAND_FEET_TOP_FRAC` (0.85, ≈ fence line) and `CONFIG.GROUND_BAND_FEET_BOTTOM_FRAC` (0.99, ≈ canvas floor). Two new tunable layout constants in `js/config.js`. Sub-task bottom-alignment untouched. Files: `js/config.js`, `js/movement.js`, `test/movement.test.js` (two full-canvas assertions rewritten to the band + a new full-jitter-range bounds test), `docs/UI_UX.md`, `docs/MECHANICS.md`, `docs/ROADMAP.md` (checked off), `docs/DECISIONS.md`. Commit pushed by Jeremy.

**The overlap judgment call (what the split was for):** narrowing the range clusters items vertically, risking same-timeline enemies (identical due time → identical x) stacking exactly. The ~56px jitter (14% of a 401px canvas) resolves it — same-x items land at staggered feet heights. Live-verified: two 11:59 PM tasks spawned at x=655 with feet at frac 0.893 and 0.961 (both in-band, 27px apart); a 2,000-sample sweep landed every foot in [0.85, 0.99]. Judged acceptable — a real ground-line crowd overlaps somewhat anyway, and exact-same-time collisions are the uncommon worst case.

**State:** 60 suites, 1271/1271 (+1). **Jest ran clean from `$HOME` this session** — the prior session's sandbox jest wall did NOT recur (try it normally next session; if it breaks again, `node --check` + live Chrome is the fallback). Live-verified in Chrome against Jeremy's real `node server.js`: single restored zombie grounded at feet frac 0.961; two-zombie same-x stagger confirmed by measurement + screenshot; zero console errors. Jeremy's real save left untouched (1 task, XP/Points 10, Health 100) — the two-zombie test task was removed cleanly afterward (see Watch out).

**Next:** Per the V1 handoff's standing queue — V2 (state-indicator glow redesign) → V3b (portrait glance strip) → V3c (landscape scene + tap-to-interact); V4 (AI-generate pixel church) anytime; V5 (design tokens) anytime; V6 (juice pass) waits on Jeremy's play week. Also still open, unscheduled: the `uncompleteItem` overdue back-charge sibling hazard (needs Jeremy's design call, ROADMAP Known bugs) and re-checking the shop's "days to afford X" prose against real play data.

**Watch out:**
- **The ground-band fractions (0.85/0.99) are tuned against the V1 scene's CSS `.scene-ground` (bottom 24%, fence near its top).** If the ground band's height or the fence position in `css/gameCanvas.css` ever changes, re-check these two `CONFIG` values — they're matched to the CSS by hand (deliberately NOT coupled to the DOM rect; `getItemTopPosition` only receives `canvasHeight`). They're tunable and layout-only, not economy balance.
- **Persistence/saveGame/State are all closure-scoped — NOT on `window`.** The documented "stub `Persistence.flush`/`requestSave` before a direct localStorage edit" trick silently no-ops here (the `if (window.Persistence)` guard is false). To hand-edit `localStorage` and survive a reload, block the write at the source instead: override `localStorage.setItem` to drop writes to the `deadline.save` key, THEN navigate — the unload-flush's `setItem` gets swallowed and your edit persists. (First reload attempt this session failed exactly because the stub missed and the unload-flush re-serialized live state over the edit.)
- Effects are currently set to **Off** in Jeremy's settings (`fx-off` body class) — that's why idle cloud drift/fog/walk-bob don't animate. Not a bug. Toggle via Settings → "Streak Fire Effects" → Full/Reduced.
- Same SW-unregister + Cache-Storage-clear-then-navigate dance was needed to pick up the `movement.js`/`config.js` edits against the live dev server — a plain reload didn't.

## 2026-07-21 — V1 shipped: graveyard atmosphere + slider-coupled parallax (Cowork session, Sonnet)

**Did:** Closed VISUAL_DESIGN_PLAN.md's V1 item, next in the standing visual-track queue (fully spec'd by the earlier 2026-07-21 art-direction Fable session). New `.graveyard-scene` layer stack behind `#base`/`#heroBaseZone`/`#midnightLine`/`.enemy` in `#gameCanvas` (`css/gameCanvas.css`, `index.html`): night-sky gradient + static moon glow, faint clouds, a far hill + 3 bare-tree silhouettes and a fence + 3 tombstones (both inline SVG, no new binary asset needed), midnight line restyled from the old red alarm hairline to a dark iron gate-post with a lantern glow. Slider-coupled parallax: `js/ui/timeSliderView.js` sets a new `--scene-progress` CSS var (0–1, minutes-of-day) on every live tick AND scrub, read by `.scene-clouds`/`.scene-silhouettes`'s `translateX(calc(...))` at different rates (clouds fastest, silhouettes slower); `.scene-ground` deliberately stays untransformed (fixed, per the art-direction decision — the stillness against the moving layers is what sells the depth). Idle cloud drift + ground fog are separate keyframe animations gated behind `fx-off`/`fx-reduced` + `prefers-reduced-motion`, matching the existing streak-fire/walk-bob pattern. `sw.js` CACHE_NAME bumped v1→v2 (index.html/gameCanvas.css/timeSliderView.js all changed; no new cacheable files since props are inline SVG). Updated `docs/UI_UX.md` (new section), `docs/VISUAL_DESIGN_PLAN.md` (checked off), `docs/ROADMAP.md`, `docs/DECISIONS.md`.

**Found + fixed live in Chrome (not caught by static review):** the scene was completely invisible on first load despite every layer having correct computed styles/rects — `#gameCanvas` needed an explicit `z-index: 0` to establish its own stacking context, otherwise the scene's negative-z-index layers (intentionally negative so no OTHER element's z-index had to change) escaped upward past `#gameCanvas` entirely instead of staying scoped behind its children. One-line fix; logged in UI_UX.md as a general gotcha for any future negative-z decorative layer.

**Deliberately NOT done — split per the task's own stop condition:** "zombies bottom-aligned to the ground band" only shipped as a visual band; zombies don't actually stand on it yet. Checked before writing CSS: `Movement.getItemTopPosition` randomizes a top-level item's vertical position across the FULL canvas height, not a bottom band — narrowing that range is real lane-math (affects multi-item overlap), which is exactly the "STOP and split, that's movement.js" condition the V1 task itself called out. Logged as its own ROADMAP follow-up rather than decided unilaterally in a CSS-scoped session.

**Live-tuning pass (same session, several rounds of Jeremy's direct feedback in Chrome — all `css/gameCanvas.css` + one small `js/ui/timeSliderView.js` addition):**
1. Clouds brightened + parallax rates widened (clouds -9%→-18%, silhouettes -3%→-7%) so the slider-driven motion actually reads.
2. **Ground now moves too, and fastest of all layers** (was intentionally fixed per the original art-direction call) — Jeremy's observation: zombies stand on the ground band and visibly move as you scrub, so a perfectly static ground under them read as sliding rather than walking. This also happens to match REAL parallax physics (closest layer moves most) rather than the original stylized inversion. Rate: `-22%` (steeper than clouds' `-18%`), with `.scene-ground` widened to `130%/-15%` (matching the clouds' margin trick) so the bigger swing never reveals a gap at the canvas edge. **This reverses the "ground fixed" decision logged earlier today — if it holds up, treat that DECISIONS.md entry as superseded, not the current state.**
3. Idle cloud "wind drift" added, tuned across 4 rounds: brighter → sped up 70s→12s → rebuilt from a `transform`-based wiggle (bounded, snapped back visibly) into a seamless `background-position` loop on a repeating tile (sails off the right edge, reappears on the left, no pop) → tile widened 500px→1100px with fewer shapes (4→2) per Jeremy's "too many clouds" call, with duration scaled 7s→15s in the same step to hold the px/sec speed constant through the density change. `js/ui/timeSliderView.js`'s `handleScrubInput`/`handleRelease` now toggle a `.parallax-scrubbing` class on `#graveyardScene` so the idle drift PAUSES for the duration of an active scrub (`animation-play-state: paused`) and resumes on release — keeps the idle "wind" and the slider-driven parallax from ever visually fighting each other.

**State:** `node --check` clean on `js/ui/timeSliderView.js`; module load-verified via a direct `require()` (no syntax/runtime error). **`npx jest` could not be run this session** — a new sandbox-environment finding, unrelated to this session's code: even a from-scratch minimal `jest.config.js` + trivial test in a brand-new sandbox directory fails jest's own config-validation resolver for a `setupFilesAfterEnv` file that `fs.existsSync`/`require()`/`graceful-fs` all independently confirm exists and is readable. Reproduced outside this repo entirely, so it reads as sandbox infrastructure, not a regression — but it means today's verification leaned entirely on `node --check` + live Chrome rather than the usual green-suite number. Flag if a future session hits the same wall. Live-verified end-to-end in Chrome against Jeremy's real `node server.js`: desktop scene renders correctly (hill/trees/fence/tombstones/moon all visible, matches the "playful, slightly eerie, never grim" direction); 390×844 same-origin iframe showed zero page-level overflow; `--scene-progress`/`translateX` swept correctly across the full 0–1439 minute range via direct slider-value testing (ground stayed untransformed throughout, confirming "ground fixed"); `fx-off`/`fx-reduced` both correctly stopped the idle animations, default left them running; hero-chip zone (base's leftmost 120px) unchanged; zero console errors beyond the standard Chrome-extension message-channel noise.

**Next:** Jeremy's explicit next ask (start of next session): "getting the sprites to show" — NOT yet scoped/root-caused this session, no context gathered on it beyond the name. First step next session is to pin down exactly what "sprites not showing" means (which sprites, when, repro steps) before touching code. Separately still queued: the movement.js follow-up (put zombies on the new ground band — narrow `getItemTopPosition`'s random-top range, check multi-item overlap before/after), then the standing visual-track queue: V2 (state-indicator glow redesign) → V3b (portrait glance strip) → V3c (landscape scene + tap-to-interact, now unblocked since V1's scene exists); V4 (AI-generate pixel church) anytime; V5 (design tokens) anytime; V6 (juice pass) waits on Jeremy's play week. Also still open, unscheduled: the `uncompleteItem` overdue back-charge sibling hazard (needs Jeremy's design call) and re-checking the shop's "days to afford X" prose against real play data.

**Watch out:**
- Any FUTURE negative-z-index decorative layer needs its positioned ancestor to have an explicit non-`auto` `z-index` (even just `0`) or it will silently fail to render exactly like V1's first pass did — correct computed styles/rects everywhere, nothing visible on screen. Don't trust "the DOM looks right" as sufficient verification for a layered scene; screenshot it.
- Jest is currently unusable from this Cowork sandbox session (see State above) — reproduced with a trivial fresh project, not specific to this repo's config. Try again fresh next session before assuming it's still broken; if it recurs, `node --check` + live Chrome is the fallback verification path this session used successfully.
- `Movement.getItemTopPosition`'s full-canvas-height random range is now visually mismatched with the new ground band (zombies can render "floating" above it) until the follow-up movement.js session lands — cosmetic only, not a functional regression, but expect it to look odd in a live session before that follow-up ships.
- **"Ground fixed" is no longer accurate** — `.scene-ground` moves now (fastest of all layers, see the tuning-pass paragraph above). If a future session reads the earlier "ground fixed, sells the depth differential" reasoning in DECISIONS.md/UI_UX.md without also reading this entry, it'll be working from a superseded assumption. UI_UX.md/DECISIONS.md text below wasn't rewritten line-by-line to match (would require re-deriving exact numbers from live CSS) — treat this HANDOFF entry as the source of truth for current values (clouds -18%, silhouettes -7%, ground -22%) until a future doc pass reconciles them.
- Repeat the SW-unregister-then-navigate dance (documented in CLAUDE.md) before trusting any visual check against Jeremy's live server this session needed it every single CSS iteration, not just once.

## 2026-07-21 — V3a shipped: agenda rows no longer clip at 390px; Reset Game gated to dev mode (Cowork session, Sonnet)

**Did:** Closed VISUAL_DESIGN_PLAN.md's V3a item, next in the standing visual-track queue. (1) Agenda-row clipping at 390px: fixed in two separate passes after live measurement showed the first pass wasn't actually clean — `.item-info`'s internal title/details rows (unclassed JS-built divs, targeted structurally via `.item-info div { flex-wrap: wrap }` in `css/responsive.css`, plus `min-width:0`/`overflow-wrap:break-word` on `.item-name` in `css/agendaList.css`, plus `flex-shrink:1 !important` on `.task-controls` to beat its inline `flex-shrink:0`), then a SECOND chain found only via `getBoundingClientRect` measurement: sub-tasks render as a sibling of `.item-info` (not a descendant), so their own unwrapped row (`.sub-task-item`/`.sub-task-info`/`.sub-task-controls` — classed, no structural selector needed) was still dragging the whole card wider. (2) Reset Game (moved into Settings earlier today, see the entry below) was unconditionally visible to every player on the live build — new pure `SettingsView.isDevMode(loc)` gates it to `localhost`/`127.0.0.1` or a `?dev` query param; exported + unit-tested (`test/settings-view-devmode.test.js`, +6). Updated `docs/UI_UX.md`, `docs/VISUAL_DESIGN_PLAN.md` (checked off), `docs/DECISIONS.md`.

**State:** 60 suites, 1270/1270 (+6). `node --check` clean on `js/ui/settingsView.js`; CSS brace-balance checked on both touched files. Live-verified in Chrome against Jeremy's real `node server.js`: a same-origin 390×844 iframe (standing mobile-verification recipe) showed a real task + real sub-task rendering with zero clipping, confirmed quantitatively via `scrollWidth`/`clientWidth`/`getBoundingClientRect` on the row/container/body/html (zero overflow anywhere except `.game-canvas`, which is pre-existing/out-of-scope, queued separately under V1/V3b). Settings → Reset Game correctly shown on localhost; `isDevMode` correctly computed `false` for a simulated production hostname with no `?dev` param; Reset itself still works end-to-end. Dev save reset to pristine afterward. Zero console errors.

**Next:** V1 (parallax graveyard) → V2 (state-indicator glow redesign) → V3b (portrait glance strip) → V3c (landscape scene + tap-to-interact); V4 (AI-generate pixel church) anytime — art source already unblocked (2026-07-21 art-direction session); V5 (design tokens) anytime; V6 (juice pass) waits on Jeremy's play week. Also still open, unscheduled: the `uncompleteItem` overdue back-charge sibling hazard (needs Jeremy's design call, see ROADMAP Known bugs) and re-checking the shop's "days to afford X" prose against real play data now that points are ~10x smaller.

**Watch out:**
- The `.item-info div` / `.sub-task-item` flex-wrap fixes are BOTH scoped inside `css/responsive.css`'s existing `max-width:768px` block — desktop is untouched by design. If a future session adds new unclassed flex rows inside `.item-info` (or a new sibling section next to it, like `.sub-tasks-section`), they will NOT automatically inherit this fix — check for the same nowrap-flex-row overflow pattern before assuming CSS-only rows are safe at mobile widths.
- A "looks clean in a screenshot" check is NOT sufficient for this class of bug — the first pass here visually looked fine but still measured 49px of real overflow. Use `el.scrollWidth > el.clientWidth` (or `getBoundingClientRect` comparisons against the parent) as the actual pass/fail signal, not just eyeballing a screenshot.
- `isDevMode`'s `?dev` escape hatch works on ANY origin including the live GitHub Pages build — intentional (Jeremy can still spot-check Reset there), but means Reset is technically reachable by anyone who knows to append `?dev` to the URL. Acceptable per V3a's scope (this is dev chrome, not a security boundary); flag if that assumption ever needs to change.

## 2026-07-21 — Overdue-edit back-charge bug fixed: editing a due date into the past no longer replays missed damage intervals (Cowork session, root cause + design resolved 2026-07-20, Sonnet execution)

**Did:** Closed the last unguarded entry of the 2026-07-18 far-past-clock bug family (ROADMAP Known bugs). Root cause: `Items.markAsOverdue(item, currentTime, deps)` has always ignored its own `currentTime` parameter — it unconditionally parks `item.lastDamageTickTime = item.dueDateTime.getTime()`. `spawning.js`'s created-already-overdue path already worked around this with a one-line override (`itemData.lastDamageTickTime = Date.now()`) right after calling `markAsOverdue`; `recomputeOverdueStateAfterEdit`'s pulled-into-the-past branch (fires when an edit moves a due date backward) never got the same treatment, so a same-day 10PM→9AM edit dealt −92 HP in seconds. Fix mirrors spawning.js's exact pattern: `item.lastDamageTickTime = now.getTime()` immediately after `markAsOverdue(item, now, deps)` in that branch. Both design forks were already resolved 2026-07-20 (Jeremy, Fable) — this was pure Sonnet execution. New `test/overdue-edit.test.js` (6 tests). Updated `docs/ROADMAP.md` (checked off), `docs/MECHANICS.md` (new Overdue Damage bullet), `docs/DECISIONS.md`.

**Found, not fixed:** `uncompleteItem` (items.js:1043) has the identical latent pattern when undoing a completion on a past-due item — logged as a new unchecked ROADMAP Known bugs entry rather than silently applying the same fix, since "undo a completion you knew was overdue" may deserve different treatment than an edit and wasn't part of the original bug report. Needs Jeremy's call before touching.

**State:** 59 suites, 1264/1264 (+6). `node --check` clean on items.js. Live-verified in Chrome against Jeremy's real `node server.js`: created a task due in the future, edited its due time to ~12 hours in the past, confirmed Health stayed at 100 on landing (previously would have dropped 30+ HP), and confirmed via `State.getActiveItems()`/`State.getBaseHealth()` in-page that `lastDamageTickTime` sat ~13 seconds in the past (the edit moment) while the due date itself was ~12 hours back — the clock demonstrably did NOT get backdated to the due time. Zero console errors. Dev save reset to pristine afterward via the in-app Settings → Reset Game flow.

**Next:** Unchanged from prior entries — V3a (agenda rows clip at 390px) → V1 (parallax) → V2 → V3b → V3c next on the visual track; V4 (AI-generate pixel church) anytime; V6 waits on the play week. The newly-found `uncompleteItem` sibling hazard is unscheduled — needs a design decision from Jeremy before it becomes a ticket. Also still open from two sessions ago: re-check the shop's session-24 balance rationale against real play data once Jeremy has some — every "days to afford X" figure is now ~10x larger post the points re-tune.

**Watch out:**
- `markAsOverdue`'s `currentTime` parameter is STILL a dead parameter at its other 3 call sites (loop.js, damage.js, items.js:1043's uncompleteItem) — only the edit path got the override this session, deliberately scoped narrow (see DECISIONS.md's "alternatives considered"). Don't assume the function itself was fixed generically if you go looking at it later.
- Same SW/Cache-Storage-clear-then-hard-navigate dance was needed again to pick up this session's items.js edit against Jeremy's live dev server — a plain reload alone did not appear to pick up the change reliably; the standing recipe in CLAUDE.md still applies.

## 2026-07-21 — Points economy re-tuned 10x down; sub-tasked parents pay 0 pts of their own; habit rate bonus re-anchored to a 22-occurrence window (Cowork session, Jeremy's call throughout, Sonnet execution)

**Did:** Balance-tuning protocol session, Jeremy's request. `CONFIG.POINTS_PER_TASK`/`POINTS_PER_HABIT`/`SUBTASK_POINTS` 10/5/5 → 1/1/1 — shop prices (25-300 pts) deliberately left unchanged so tokens (freezes, pushbacks, cheat days) cost real sustained effort instead of a day's grinding; XP untouched (still 10/5), so leveling pace doesn't move. New rule in `Items.completeItem`/`uncompleteItem`: a parent task that ever had a sub-task (`completedSubTasks > 0`) now earns 0 points of its own on final completion — all the points for a sub-tasked task come from completing its subs, not from checking off the parent (XP unaffected). Habit rate bonus re-anchored off the old "task parity" model (meaningless once tasks are worth 1) to Jeremy's "22 reps make a habit" framing: `HABIT_RATE_WINDOW`/`HABIT_RATE_MIN_SAMPLE` 14/7 → 22/22 (no tier fires until a full 22-occurrence history exists), `HABIT_RATE_TIERS` 90%/70%→2×/1.5× → 100%/80%→3×/2×. All three design calls (sub-task rounding/behavior, habit tier redesign, min-sample gating) were Jeremy's explicit choices via clarifying questions before implementation — see DECISIONS.md for the full rationale and rejected alternatives. Updated `js/config.js`, `js/items.js`, `docs/ECONOMY.md`, `docs/DECISIONS.md`, `docs/ROADMAP.md`, and 3 test files whose expectations were pinned to the old numbers/behavior (`test/subtask-economy.test.js` rewritten with 2 new tests, `test/habits.test.js`'s tier tests rebuilt around the 22-window, `test/items-indulge.test.js`'s one fixture balance adjusted). Jeremy also hand-updated `.claude/skills/balance-tuning/SKILL.md`'s canonical values list himself (protected from Cowork edits) after one round of feedback on a merge slip.

**State:** 58 suites, 1258/1258 (+2) in the sandbox. `node --check` clean on config.js/items.js. Live-verified end-to-end in Chrome against Jeremy's real `node server.js`: a standalone task paid exactly 10 XP/1 pt; a sub-task paid 5 XP/1 pt and then completing its parent paid +10 XP/+0 pts (points-per-item math confirmed live, matching the new rule exactly); a brand-new habit (below the 22-sample minimum) paid the flat 5 XP/1 pt base. Zero console errors throughout. Dev save reset to pristine via the in-app Settings → Reset Game flow (built last session) afterward.

**Next:** Unchanged from the prior entries — overdue-edit bugfix first (small, fully specified in ROADMAP Known bugs), then V3a → V1 (parallax) → V2 → V3b → V3c; V4 (AI-generate pixel church) anytime; V6 waits on the play week. Separately, worth flagging for a future session: the shop's session-24 balance rationale (HP-per-point tiers, break-even math) still holds in RATIO terms but every "days to afford X" figure in ECONOMY.md's prose is now ~10x larger — re-check against real play data once Jeremy has some, same standing follow-up as before, just a bigger gap now.

**Watch out:**
- The habit rate bonus now requires a FULL 22 recorded occurrences before any multiplier can fire (both the 2× and 3× tiers) — a habit started today won't see its first possible bonus for 22 scheduled occurrences (up to 22 days out for a daily habit). This is intentional (Jeremy's "22 reps" framing) but will look like "the bonus is broken" to anyone who doesn't know the design — flag it if it comes up in a future bug report.
- `.claude/skills/balance-tuning/SKILL.md` is NOT editable from the Cowork sandbox (Edit tool blocks it as a protected path) — any future canonical-values update from a Cowork session needs to be handed to Jeremy as exact text to paste himself, same as this session.
- Achievement thresholds, the debt/break-even HUD nudge, and shop prices were deliberately NOT touched — the break-even nudge already derives its "complete N tasks" figure from `pointsPerTask` dynamically so it reads correctly at any value with no code change, it'll just show bigger N now.

## 2026-07-21 — Reset button moved to Settings + in-app confirm; FAB menu restyled for contrast (Cowork session, Sonnet)

**Did:** Two small/independent UI fixes, batched per the revised guardrail (no persistence/architecture/balance touched). (1) Moved the dev-only "↺ Reset" button from a floating bottom-left corner spot on the main game screen into a new "Reset Game" section at the bottom of the Settings window (`js/ui/settingsView.js`) — deleted `index.html`'s `#resetTestButton` + `css/base.css`'s `.reset-test-button` rules; wired `onConfirmReset` through `managementWindows.js`'s existing settings deps pass-through into script.js's unchanged `handleConfirmReset` (same wipe logic as before, just moved/renamed). Also swapped the confirmation from the browser's native `confirm()` to an in-app `Modal.open` dialog matching Import's existing replace-confirm pattern (new `.settings-danger-section`/`.danger-button`/`.reset-confirm-modal` CSS in `css/settings.css`) — removes the documented CDP-freeze risk for testing this flow. (2) Restyled `css/fabMenu.css`'s `.fab-menu-item` (the six FAB popout buttons) from white/pale-green-border to the same dark-green gradient as the FAB button itself, white text/icons, stronger shadow — Jeremy's report was "light buttons on a light background, hard on the eyes." Docs updated: UI_UX.md (new section), DECISIONS.md (one entry covering both fixes).

**State:** 58 suites, 1256/1256 (unchanged — no test file touched, this was DOM/CSS wiring only). `node --check` clean on script.js/settingsView.js/managementWindows.js. Live-verified end-to-end in Chrome against Jeremy's real `node server.js`: floating Reset button gone; FAB menu screenshot confirms high-contrast dark-green items; Settings → Reset Game opens the in-app confirm dialog (no CDP freeze); Cancel closes only the dialog; a real task created through the UI was fully wiped by Reset → confirm (board returned to onboarding empty state, persisted save's `activeItems` read back `[]`). Zero console errors. Dev save left in its post-reset pristine empty state.

**Next:** Unchanged from the prior entry (art-direction session) — overdue-edit bugfix first, then V3a → V1 (parallax) → V2 → V3b → V3c; V4 (AI-generate pixel church) anytime now; V6 waits on the play week.

**Watch out:**
- Playtesting from Cowork against a live localhost server sometimes serves a stale cached build even on a plain reload — had to unregister the service worker + clear Cache Storage via `javascript_tool` before a normal `navigate` picked up this session's HTML/CSS edits (index.html markup + 3 CSS files). Don't assume a plain reload is enough to verify Cowork-side edits against Jeremy's running server; the SW-unregister-then-navigate sequence documented in CLAUDE.md for the Pages build applies here too.
- `server.js` 404s on any URL with a query string (tried `?nocache=1` as a cache-buster) — use the SW/Cache-Storage-clear approach above instead of query-string busting against this dev server.
- The Settings window's Reset section reuses `Modal.open`'s `dedupeSelector` guard (`.reset-confirm-overlay`) same as Import — if a future session adds a third destructive confirm to Settings, follow the same naming pattern (`.<action>-confirm-overlay`) rather than reusing Import's or Reset's classes.

## 2026-07-21 — Art direction locked: pixel stays, parallax in, tilt out; V4 unblocked (Cowork session, Fable)

**Did:** Design session, NO code changes. Researched the 2026 AI game-art tool landscape (web) and resolved three art-direction questions with Jeremy: (1) **pixel art locked** as the permanent style — his visibility worry (sprites lost over a pixel background) was diagnosed as a contrast problem V1's dark silhouette background already solves, not a style problem; (2) **slider-coupled parallax layers added to V1's scope** (per-layer `translateX` rates off the slider value — clouds fastest, ground fixed — so scrub direction reads visually; idle drift fx-gated); (3) **camera tilt on scrub REJECTED** (rotation breaks the pixel grid; parallax delivers the feel; revisit-only-after-V1 terms in DECISIONS.md). Locking pixel also resolved V4's blocker: church art source = **AI-generate** (Retro Diffusion for base sprites with the current renders as style reference; PixelLab for animation/rotation frames — the current de facto pixel stack, and the reason Jeremy's 2025 animation attempt failed is now solved *for pixel art specifically*). Vector question answered: raster PNG only, SVG stays scene-props-only. Docs updated: DECISIONS.md (one 3-part entry), VISUAL_DESIGN_PLAN.md (V1 scope, V4 unblocked, order note), ART_STYLE.md (locked-style note + new AI Asset Pipeline section).

**State:** Code untouched — still 58 suites / 1237/1237 from 2026-07-19 (not re-run; no `.js`/`.css` edited). Live Pages build and dev save untouched.

**Next:** Unchanged from yesterday, minus the V4 blocker: overdue-edit bugfix first (small, fully specified in ROADMAP, good Sonnet opener), then V3a → V1 (now incl. parallax) → V2 → V3b → V3c; V4 anytime now (AI-generate pixel church — a good session is generating the 5 base states in Retro Diffusion and swapping the files in). V6 still waits on the play week.

**Watch out:**
- V1's executing session must implement parallax as transform-only layers driven by the existing slider value — do NOT add rotate transforms anywhere in the scene (tilt was explicitly rejected, DECISIONS.md 2026-07-21).
- When V4 executes: same 5 filenames (`base_000..100.png`) so zero code churn; keep the hero-chip zone (base's leftmost 120px) clear per the standing audit hazard.
...
**Real bug found + fixed live:** the CTA button opened the Tasks window then immediately closed it again on the same click — root-caused via a `classList` trace to `Modal.js`'s global dismiss-click handler, which only exempts `.management-window`/`.fab-container` from "click outside closes the open window." The CTA lives in neither, so its own click bubbled to `document` and closed what it just opened. Fixed with `e.stopPropagation()` on the CTA listener (not a new Modal.js exemption — kept the fix local to the one-off trigger). Also updated `test/agenda-list.test.js`'s shared fake-DOM `fire()` helper to pass a minimal fake event, needed for the new stopPropagation call, harmless for existing tests.

**State:** 58 suites, 1237/1237 (+10). `node --check` clean on script.js/agendaList.js. Live-verified in Chrome end-to-end against a genuinely first-run save (localStorage cleared with Persistence neutered first, then a real hard reload to rule out HTTP-cache staleness — a soft reload alone was NOT enough here, see Watch out): onboarding renders correctly, FAB pulses, CTA opens Tasks and stays open post-fix, a real task creation clears both the onboarding block and the FAB hint, and completing that task back to empty correctly shows the PLAIN neutral empty state (not onboarding) since completedItems is no longer empty — confirms "true first-run" and "returning player, empty day" stay distinct. Zero real app console errors. Dev save is now a fresh save with one real task/completion cycle from verification — left in place, harmless.

**Next:** Milestone 5 open again. One candidate left unqueued from the original scoping session: phone deployment (hosting beyond localhost, needed for real daily-play data generation). Fresh scoping is also fine.

**Watch out:**
- **New standing gotcha for any future trigger living OUTSIDE `.fab-container`/`.management-window` that opens a management window:** `Modal.initDismissHandlers`'s document-level click listener will close it again on the same click unless you `e.stopPropagation()` on that trigger's own listener. The FAB menu never hits this because it's inside the exempted container; anything new that opens a window from elsewhere in the page will.
- **A plain SW-cache-clear + soft navigate was NOT sufficient to pick up a script.js edit this session** — had to fall back to a real `ctrl+shift+r` hard reload (browser HTTP/disk cache, distinct from both the Service Worker Cache Storage this repo's standing rule already covers AND the localStorage-write-then-reload race). Suspect the HTTP disk cache when a code change doesn't seem to take effect despite clearing SW state.
- The localStorage-clear-then-reload sequence needed the SAME `Persistence.requestSave`/`flush`-neutering-before-the-write trick as the session-52/Export-Import precedent, confirmed again here — a plain `localStorage.removeItem` immediately before navigating was NOT enough on its own; the live page's own autosave/beforeunload hooks re-wrote the old save back before the navigation actually took effect the first time this was tried.

## 2026-07-20 — UX fixes batch shipped: all 4 playtest findings fixed (Cowork session, Sonnet)

**Did:** Closed the "UX fixes batch" ROADMAP item queued in the prior scoping session (all 4 findings, one session per the batching guardrail — none touch persistence/architecture/balance). (1) Unified script.js's two duplicated `updateTaskCountDisplay` inline closures into one real function + new `DayPagerView.refreshWeekStrip()`, so the week strip now live-refreshes on every task/habit create/complete/uncomplete instead of only on load/pager nav. (2) `js/ui/forms.js`'s new pure `computeDefaultDueTime(now)` replaces the hardcoded 5:00 PM Add-Task default: max(5:00 PM, now+1h rounded up to next half-hour), capped 11:59 PM same day — kills the evening instant-overdue spawn. (3) `dayPagerView.js`'s `render()` sets a day-scoped task count on ghost/snapshot pages, restores the real global count on return to Today. (4) New `.day-pager-hidden` CSS class hides "Completed Today" while viewing any non-Today pager page. Updated docs/ROADMAP.md (checked off), docs/UI_UX.md (3 new notes: Agenda List header, Add Task default, Completed Today, Week strip), docs/DECISIONS.md.

**State:** 58 suites, 1227/1227 (+9, `test/forms-due-time-default.test.js`). `node --check` clean on script.js/forms.js/dayPagerView.js. Live-verified in Chrome end-to-end (SW cache cleared first): real task creation updated the week strip's Today count 2→3 with zero pager navigation; Add Task at a real 8:13:25 PM pre-filled exactly 9:30 PM; paging to Tomorrow showed "1 task" (was stale "2 tasks") with Completed Today fully hidden; paging back to Today restored both correctly. Zero real app console errors (only the standing Chrome-extension message-channel noise). Dev save now also has a "Fix verify task" (due 9:30 PM) added during this session's live verification, left in place — harmless for a dev save.

**Next:** Nothing else queued in Milestone 5 — genuinely open again. Two candidates were considered but deliberately not queued in the scoping session: phone deployment (hosting beyond localhost, needed to actually start generating real daily-play data) and a first-run/onboarding experience (fresh save currently drops the player on an empty board with zero guidance). Either is a reasonable next Milestone 5 pick, or a fresh scoping pass.

**Watch out:**
- `DayPagerView.refreshWeekStrip()` is safe to call before `DayPagerView.init()` (guards on `deps === null`) — relevant because `updateTaskCountDisplay()` can fire during `initGame()`/`restoreGameState()`, which run before script.js's `DayPagerView.init(dayPagerViewDeps())` line.
- The `.day-pager-hidden` class is intentionally separate from the shared `.hidden` utility AgendaList's `renderCompletedItems` also toggles — don't consolidate them without re-checking both owners' toggle conditions first (see DECISIONS.md for why they'd fight over one flag).
- `setDayScopedTaskCount`/day-scoped hiding logic lives inline in `dayPagerView.js`'s `render()`, not as its own pure/tested function — no Jest coverage of this specific wiring path (matches the existing pattern where `render()` itself is untested; only `formatDayLabel` has direct coverage in `day-pager-view.test.js`).
...
**Two real bugs found live in Chrome, both fixed same session:**
1. **Cross-module wiring gap** — script.js's `openManagementWindow` deps gained `buildExportEnvelope`/`currentSummary`/`onConfirmImport`, but `js/ui/managementWindows.js`'s `'settings'` dispatch branch wasn't forwarding them to `SettingsView.renderSettingsWindow`, throwing `TypeError: deps.buildExportEnvelope is not a function` the moment Export was clicked. No Jest coverage catches this class of gap (script.js is never `require()`'d, and this specific pass-through has no unit test) — only live console reading caught it, per the standing rule from the sub-session-3 regression below.
2. **Async-reload race** — `window.location.reload()` is asynchronous; a pending debounced `Persistence.requestSave`, the 5s `CONFIG.PERSISTENCE_AUTOSAVE_MS` autosave, or the beforeunload flush hook could fire in the gap between the raw localStorage write and the actual navigation, re-serializing the LIVE pre-import state right back over the freshly-imported save. Confirmed via a live round-trip test with distinctive synthetic values (Health 77/XP 999/Level 7/Points 12345) — the HUD still showed the OLD values post-reload, and `localStorage.getItem('deadline.save')` matched the pre-import backup exactly. Fixed by neutering `Persistence.requestSave`/`Persistence.flush` to no-ops immediately before the write — same class of hazard as the session-52 "stub flush/requestSave before a direct localStorage edit" trick already in CLAUDE.md's Cowork section, now applied to the reload-transition case too.

**State:** 57 suites, 1218/1218 (+13 new `test/exportImport.test.js`, everything else unchanged). `node --check` clean on all 5 touched/new files. Live-verified in Chrome end-to-end AFTER both fixes: clean boot zero console errors, Settings window renders the new section correctly, Copy-to-clipboard succeeds, a full paste→validate→confirm-modal(compare table correct)→confirm→reload→exact-byte-for-byte-restore round trip (including settings), and all rejection paths (truncated/garbage JSON, newer-schemaVersion, newer-exportFormatVersion; checksum-mismatch covered in Jest) render the correct inline error with no modal opened. Backup key correctly held the pre-import values throughout.

**Next:** Nothing else queued in Milestone 5 yet — this was picked directly from Jeremy's request rather than from a scoped roadmap item. Broader Milestone 5 scoping (Fable) is still genuinely open, same as the prior session's handoff noted.

**Watch out:**
- **New standing pattern for ANY future feature that writes localStorage then reloads the page:** neuter `Persistence.requestSave`/`Persistence.flush` to no-ops immediately before the write, not just at the read side (the session-52 trick was documented for reads-after-writes on a still-live page; this session found the SAME race exists on the write-then-reload path too, via the async gap `location.reload()` leaves open).
- **Cross-module deps pass-through has zero test coverage** — when adding new deps fields to a management-window dispatch (script.js → managementWindows.js → a *View.js render function), grep for the SAME field names on both sides of the handoff, not just where they're read. Jest never catches a dropped pass-through since script.js isn't required by any test.
- SW cache must still be cleared before every fresh navigation used for verification (standing rule from session 74/the entry below) — did this before every reload this session.
